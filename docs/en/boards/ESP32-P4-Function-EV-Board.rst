...
The `ESP32-P4-Function-EV-Board`_ is one of Espressif's official development boards, based on the `ESP32-P4`_ SoC.

.. note::
    This page documents the **v1.5.2** revision of the board. The older v1.4 revision uses a CP2102N USB-to-UART bridge instead of the built-in USB Serial/JTAG port and has a different USB port layout. See `v1.4 Board Differences`_ for details.
...
USB Ports on the Board (v1.5.2)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The v1.5.2 board has four USB connectors:
...
.. note::
    The v1.5.2 board does not have a USB-to-UART bridge chip, so UART0 is only accessible via the GPIO header pins (GPIO37 = U0TXD, GPIO38 = U0RXD). With **USB CDC on Boot** disabled, ``Serial.println()`` would go to UART0 (invisible over USB), while ``printf()`` output would still appear on the USB Serial/JTAG Port as usual.
...
.. list-table::
    ...
    * - Default ``CDC on Boot``
      ...
      - Enabled (no UART bridge on v1.5.2)
...
v1.4 Board Differences
^^^^^^^^^^^^^^^^^^^^^^

The v1.4 revision of the ESP32-P4-Function-EV-Board has significant hardware differences from v1.5.2:
...
.. list-table::
    ...
    * - Feature
      ...
      - v1.5.2
    * - UART bridge
      - CP2102N USB-to-UART bridge (connected to UART0)
      - None (uses built-in USB Serial/JTAG)
    * - Debug/flash port
      - USB-to-UART Port (via CP2102N)
      - USB Serial/JTAG Port (GPIO24/25)
    * - USB Full-Speed Port
      - Not available (GPIO26/27 on header only)
      - Available (Type-C connector)
    * - GPIO24/25 on header
      - Yes (J1 pins 27/28)
      - No (used for USB Serial/JTAG connector)
    * - USB 2.0 Type-C Port
      - Yes (High-Speed OTG)
      - Yes (High-Speed OTG)
    * - USB 2.0 Type-A Port
      - Yes (High-Speed OTG host)
      - Yes (High-Speed OTG host)

**Impact on Arduino USB settings:**

On v1.4, the CP2102N bridge routes UART0 to USB, so ``Serial.println()`` output is visible even with the default ``CDC on Boot = Disabled`` (``Serial`` maps to ``Serial0`` = UART0, which goes through the bridge). All four combinations of USB Mode and CDC on Boot produce visible serial output on v1.4.

On v1.5.2, there is no UART bridge, so ``CDC on Boot = Enabled`` is required for ``Serial.println()`` output to appear on USB. This is why the v1.5.2 defaults are ``USB Mode = Hardware CDC`` and ``CDC on Boot = Enabled``.
...
* `ESP32-P4-Function-EV-Board User Guide`_ (v1.5.2)
...
.. _ESP32-P4-Function-EV-Board: https://docs.espressif.com/projects/esp-dev-kits/en/latest/esp32p4/esp32-p4-function-ev-board/user_guide.html
...
.. _ESP32-P4-Function-EV-Board User Guide: https://docs.espressif.com/projects/esp-dev-kits/en/latest/esp32p4/esp32-p4-function-ev-board/user_guide.html
